...
export default function Interview() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/questions?role=${role}&language=${language}`)
        ...
        try {
          ...
          const loop = () => {
            if (videoRef.current?.readyState === 4) {
              ...
              if (now - lastMs >= 100) { // Run detection every 100ms
                lastMs = now;
                const r = detector.detectForVideo(videoRef.current, now);
                const f = r.detections.length;
                
                if (f === 0) {
                  setFaceStatus('No face detected');
                  if (now - lastPenaltyMs > 1000) { // Only penalize once per second
                    alertsRef.current++; setFaceAlerts(alertsRef.current);
                    setTrustScore(ts => Math.max(0, ts - 2));
                    lastPenaltyMs = now;
                  }
                } else if (f > 1) {
                  ...
                  if (now - lastPenaltyMs > 1000) { // Only penalize once per second
                    alertsRef.current++; setFaceAlerts(alertsRef.current);
                    setTrustScore(ts => Math.max(0, ts - 5));
                    lastPenaltyMs = now;
                  }
                } else {
  ...
  const startRecording = useCallback(() => {
    ...
    try {
      ...
      recorder.start(500);
      setPhase('recording'); setTimeLeft(90);
      let t = 90;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="card" style={{ padding: '16px' }}>
              ...
              {['Stay in frame throughout the interview', 'Speak clearly — only one person', '90 seconds maximum per answer', 'Answer in your selected language'].map(t => (
